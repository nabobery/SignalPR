  ...
  return (
    <div className="space-y-6 max-w-2xl">
      <div className="bg-zinc-900 rounded-lg border border-zinc-800 p-4">
        <h3 className="text-sm font-medium text-zinc-200 mb-2">Repository Configuration</h3>
        <p className="text-zinc-400 text-sm mb-4">
          SignalPR loads its configuration from a{" "}
          <code className="text-emerald-400 bg-zinc-800 px-1 rounded">.signalpr.yml</code> file in
          the root of your repository. This lets each project define its own review settings, agent
      ...
      <div className="bg-zinc-900 rounded-lg border border-zinc-800 p-4">
        <h3 className="text-sm font-medium text-zinc-200 mb-2">Learned Preferences</h3>

        <div className="flex items-start gap-2 mb-4 text-zinc-400 text-sm">
        ...
        {loading ? (
          <div className="flex items-center py-4">
            ...
            <span className="ml-2 text-zinc-400 text-sm">Loading preferences...</span>
          </div>
        ) : error ? (
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {preferences.map((pref) => (
                  <tr key={pref.id} className="border-b border-zinc-800">
                    <td className="py-2 pr-4 text-zinc-200">{pref.agent_type}</td>
                    <td className="py-2 pr-4 text-zinc-400">{pref.category_tag ?? "-"}</td>
                    <td className="py-2 pr-4 text-right text-zinc-200">
                      {(pref.accept_rate * 100).toFixed(1)}%
                    </td>
                    <td className="py-2 text-right text-zinc-200">{pref.total_decisions}</td>
                  </tr>
                ))}
              </tbody>
            </table>
